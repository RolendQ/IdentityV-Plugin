...
public class Vault {
    ...
    public Vault(final Player p, final Location firstLoc, final int direction, int speed, byte color) {
        // Both survivors and hunters can vault

        VaultManager.getInstance().add(p,speed+7); // Add 7 to prevent vaulting again immediately
        ...
        }.runTaskTimer(IdentityV.plugin, 0, speed/4);
    }
    ...
    public void blockWindow(final Location loc) {
        ...
        }.runTaskLater(IdentityV.plugin, Config.getInt("timers.hunter","window_blocker_length"));
    }
}
